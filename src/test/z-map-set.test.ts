...
describe("ZSet", () => {
  ...
  it("select maps values and preserves weights", () => {
    let s = new ZSet<string>();
    s = s.add("aa", 2);
    s = s.add("bbb", -1);

    const t = s.select((x) => x.length);
...
describe("ZMap", () => {
  ...
  it("ZSet.select maps to new type correctly", () => {
    let zs = new ZSet<string>();
    zs = zs.add("abc", 2);
    const selected = zs.select((x) => x.length);
    expect([...selected.getEntries()]).toContainEqual([3, 2]);
  });
});
